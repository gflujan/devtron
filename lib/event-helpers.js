...
exports.getEvents = () => {
  return Eval.execute(() => {
    ...
    socket.emit('get-project-listeners', mappedEmitters => {
      if (mappedEmitters) {
        if (mappedEmitters?.length) {
          ...
          console.warn('Event Helpers: No `mappedEmitters` present, something may have gone wrong');
        }
        ...
        console.warn(
          'Event Helpers: No `mappedEmitters` came back from the socket server, doing nothing.',
        );
      }
    });
