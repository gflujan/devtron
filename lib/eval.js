  ...
  static execute(expression) {
    ...
    if (typeof expression === 'function') {
      ...
      if (arguments.length > 1) {
        ...
        expressionToEval += `.apply(this, ${expressionArgs})`;
      } else {
        expressionToEval += '()';
      }
    }
    ...
    return new Promise((resolve, reject) => {
      window.chrome.devtools.inspectedWindow.eval(finalExpression, (result, error) => {
        if (error) {
          let errorToReturn = error;

          if (error.isException && error.value) {
            errorToReturn = new Error(error.value.split('\n')[0]);
            errorToReturn.stack = error.value;
          }
