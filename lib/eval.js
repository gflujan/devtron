class Eval {
  static execute(expression) {
    ...
    console.debug('🚀--BLLR?: ===============================================');
    const d = new Date();
    const s = `${d.getHours()}:${d.getMinutes()}:${d.getSeconds()}:${d.getMilliseconds()}`;
    console.debug('🚀--BLLR?: TIMESTAMP ->', { timestamp: s });
    console.debug('🚀--BLLR?: WHAT IS THE EXPRESSION ->', expression); // TODO **[G]** :: 🚀--BLLR?: REMOVE ME!!!
    console.debug('🚀--BLLR?: ===============================================');
  ...
  static getFileVersion(filePath) {
    return Eval.execute(filePath => {
      if (/\/atom\.asar\/(browser|common|renderer)\//.test(filePath)) {
        return process.versions.electron;
      }

      const fs = require('fs');
      const path = require('path');
      const appVersion = 'no app version yet, need to replace remote'; // require('electron').remote.app.getVersion()
